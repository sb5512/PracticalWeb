...
def turn (interface,rotation_degree, direction) :
	PI = math.pi
...
def move(interface, direction,distance) :
	circumferenceTyre = TYRE_DIAMETER * PI
...
def setSpeed(interface, speed0,speed1):
	interface.setMotorRotationSpeedReferences(VALID_MOTOR_IDS,[speed0,speed1])
    
def stopMotors(interface):
	interface.setMotorPwm(VALID_MOTOR_IDS[0],0)
	interface.setMotorPwm(VALID_MOTOR_IDS[1],0)
    
def getSonarValue(interface):
	return interface.getSensorValue(SONAR_ID)
